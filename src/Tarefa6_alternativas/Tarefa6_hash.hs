...
resolveJogo x jogo
    ...
    | otherwise = Nothing
    where caminho = fst (aux jogo x M.empty)

aux :: Jogo -> Int -> M.HashMap Jogo (Int, Maybe [Movimento]) -> (Maybe [Movimento], M.HashMap Jogo (Int, Maybe [Movimento]))
aux jogo max visitados
    | value /= Nothing && max <= fst (fromJust value) = (snd (fromJust value), visitados)
    | max < 0 = (Nothing, visitados)
    | estaResolvido jogo = (Just [], M.insert jogo (max, Just []) (snd auxInterage))
    | otherwise = (caminhoMaisCurto, M.insert jogo (max, caminhoMaisCurto) (snd auxInterage))
    where
        value = M.lookup jogo visitados
        visitadosFrente
            | value == Nothing = M.insert jogo (max, Nothing) visitados
            | otherwise = visitados
